...
import { Plugin, ItemView, TFile, TFolder, MarkdownRenderer, setIcon, Menu, PluginSettingTab, Setting, type App, type WorkspaceLeaf } from 'obsidian';

type VaultEntry = TFile | TFolder;
...
class RangerView extends ItemView {
  ...
  constructor(leaf: WorkspaceLeaf, app: App, plugin: RangerFmPlugin) {
    super(leaf);
    this.app = app;
    this.plugin = plugin;
  ...
  async setState(state: RangerState) {
...
const DEFAULT_SETTINGS: RangerSettings = {
  showPreview: true,
  showDetails: true,
};
...
class RangerSettingTab extends PluginSettingTab {
  ...
  constructor(app: App, plugin: RangerFmPlugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
